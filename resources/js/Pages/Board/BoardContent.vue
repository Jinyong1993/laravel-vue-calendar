...
export default {
  ...
  methods: {
    ...
    comment_edit(comment) {
      comment.editingNote = comment.note
      comment.editing = true
    },

    comment_edit_cancel(comment) {
    ...
    comment_update() {
      if(this.$refs.form.validate()) {
        this.$inertia.post(route('comment.update'), {
          note: this.dialogTargetComment.editingNote,
          comment_id: this.dialogTargetComment.comment_id,
        }, {
          headers: {
          'Content-Type': 'application/json'
          },
          ...
          onFinish: () => {
            this.active.commentEditDialog = false
    ...
    comment_create() {
      if(this.$refs.form.validate()) {
        this.$inertia.post(route('comment.store'), {
          ...
          onFinish: () => {
            this.active.commentCreateDialog = false
            this.snackbar.success = true
          },
    ...
    comment_delete() {
      if(this.$refs.form.validate()) {
        this.$inertia.post(route('comment.delete'), {
          ...
          onFinish: () => {
            this.active.commentDeleteDialog = false
            this.snackbar.success = true
          },
        })
      } else {
    ...
    commentDeleteTarget(targetComment) {
      this.dialogTargetComment = targetComment
      this.active.commentDeleteDialog = true
    },

    commentEditTarget(targetComment) {
      this.dialogTargetComment = targetComment
      this.active.commentEditDialog = true
    },
  },
};
</script>
...
<template>
  ...
  <CalendarLayout>
      ...
      >
        ...
        >
          <v-btn
            ...
            size="default"
            color="green"
            @click="active.commentCreateDialog = true"
          >
            作成
          </v-btn>
        </div>
          ...
          >
            ...
            >
              <v-btn
                ...
                size="small"
                color="red"
                @click="commentDeleteTarget(comment)"
              >
                削除
              </v-btn>
              <v-btn
            ...
            >
              ...
              <v-btn
                ...
                size="small"
                color="green"
                @click="commentEditTarget(comment)"
              >
                保存
              </v-btn>
            </div>
    ...
    <!-- コメント確認ダイアログ -->
    <BoardConfirmDialog
      v-model:show="active.commentCreateDialog"
      @confirm="comment_create"
    >
    ...
    <BoardConfirmDialog
      v-model:show="active.commentDeleteDialog"
      @confirm="comment_delete"
    >
      <template v-slot:body>
        削除しますか？
      </template>
    ...
    <BoardConfirmDialog
      v-model:show="active.commentEditDialog"
      @confirm="comment_update"
    >
      <template v-slot:body>
        保存しますか？
      </template>
